...
class MixtureGenerator:
    ...
    def cut_audios(self, s1, s2, sec, sr):
        cut_len = sr * sec
        len1 = len(s1)
        len2 = len(s2)
    ...
    def create_mix(self, idx, triplet, snr_levels, out_dir, test=False, sr=16000, **kwargs):
        ...
        if not test:
            s1, s2 = self.vad_merge(s1, vad_db), self.vad_merge(s2, vad_db)
            s1_cut, s2_cut = self.cut_audios(s1, s2, audioLen, sr)

            for i in range(len(s1_cut)):
                mix = self.snr_mixer(s1_cut[i], s2_cut[i], snr)
            ...
            s1, s2 = self.fix_length(s1, s2, 'max')
            mix = self.snr_mixer(s1, s2, snr)
            louds1 = meter.integrated_loudness(s1)
    ...
    def fix_length(self, s1, s2, min_or_max='max'):
        # Fix length
        if min_or_max == 'min':
    ...
    def generate_mixes(self, snr_levels=[0], num_workers=10, update_steps=10, **kwargs):
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as pool:
            ...
            for i in range(self.nfiles):
                ...
                futures.append(pool.submit(self.create_mix, i, triplet,
